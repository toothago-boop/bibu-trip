...
function tabelogSearch(q){return 'https://www.google.com/search?q='+encodeURIComponent('site:tabelog.com/hokkaido '+q+' 食べログ')}
function restaurantMaps(r){return mapsSearch(`${r.name} ${r.lat},${r.lon}`)}
function safeExternalUrl(value){try{const u=new URL(value);return ['http:','https:'].includes(u.protocol)?u.href:''}catch{return ''}}
...
function renderRestaurantResults(){
  $('#restaurantResults').innerHTML=restaurantResults.length?restaurantResults.map((r,i)=>`<article class="restaurant-card"><div class="restaurant-card-top"><span class="finder-badge">${r.distance<1?Math.round(r.distance*1000)+' m':r.distance.toFixed(1)+' km'}</span><span>${esc(r.kind)}</span></div><h3><a class="restaurant-name-link" href="${restaurantMaps(r)}" target="_blank" rel="noopener" title="在新分頁查看餐廳位置">${esc(r.name)} <span>↗</span></a></h3><p>${esc(r.address)}</p><div class="restaurant-meta">${r.cuisine?`<span>🍴 ${esc(r.cuisine.replaceAll(';','・'))}</span>`:''}${r.opening?`<span>🕒 ${esc(r.opening)}</span>`:''}</div><div class="event-actions"><a class="small-btn map" href="${restaurantMaps(r)}" target="_blank" rel="noopener">📍 Maps</a>${r.website?`<a class="small-btn official-menu-btn" href="${esc(r.website)}" target="_blank" rel="noopener">🍽 官方餐牌／網站</a>`:''}<a class="small-btn tabelog-btn" href="${tabelogSearch('「'+r.name+'」 '+r.address)}" target="_blank" rel="noopener">🔎 食べログ搵呢間</a><button class="small-btn add-restaurant" data-add-restaurant="${i}" type="button">＋ 加入行程</button></div></article>`).join(''):'<div class="empty">暫時未有搜尋結果</div>';
...
async function searchRestaurants(event){
  ...
  try{
    ...
    const all=(payload.elements||[]).map(x=>{const t=x.tags||{},lat=x.lat??x.center?.lat,lon=x.lon??x.center?.lon,name=restaurantLabel(t);if(!name||!Number.isFinite(lat)||!Number.isFinite(lon))return null;return{id:x.id,name,lat,lon,address:restaurantAddress(t),cuisine:t.cuisine||'',opening:t.opening_hours||'',website:safeExternalUrl(t.website||t['contact:website']||t.url||''),kind:t.amenity==='cafe'?'Cafe':t.amenity==='fast_food'?'輕食':'餐廳',distance:distanceKm(area.lat,area.lon,lat,lon)}}).filter(Boolean).sort((a,b)=>a.distance-b.distance);
